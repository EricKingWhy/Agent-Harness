...
**9. 交付层会把整个 SSE 响应攒到「流结束」才下发 → 线上没有打字机效果**（最费时间的一个坑）

现象：本地逐字流式，线上却是「等一长段，然后整篇答案一次性出现」。

实测（`Server: CloudStudio Gateway`，前置腾讯 EdgeOne）：

| 通道 | 响应头到达（发出后） | 帧到达跨度 |
|---|---|---|
| `POST /api/sessions`（SSE） | **44.158s**（≈ run 全长） | 326 帧全挤在 0.094s 内 |
| `GET /sessions/{id}/stream`（SSE） | **41.551s** | 326 帧全挤在 0.082s 内 |

注意这里最关键的一点：**SSE 的 200 + 响应头本身就被压到流结束才到**。
所以 `await fetch('/api/sessions')` 在 run 跑完前根本不 resolve——真实症状比
「没有打字机效果」更严重：前端在答案答完之后才拿到第一帧。换 GET 也一样（两条
通道都攒），所以「把 POST 改成 EventSource」这类改法是白费。

**别再用事件自带的 `time` 判断攒包**：`SessionEvent.time` 是**服务端生成时刻**，
不是到达时刻，拿它测会得出反向结论。要判定「App 到底有没有在流式发射」，正确做法是
run 进行中轮询服务端事件库（`GET /sessions/{id}/events`），看 `text/delta` 的**条数
是否随时间递增**——递增说明 App 无罪，攒包在投递层。

**正解：live 流改走 WebSocket 的既有通道（`/api/ws`，PRD §5.1 多路复用）。**
WS 是帧协议，交付层无法这样攒帧。同一链路上的实测对比：

| 通道 | 结果 |
|---|---|
| 服务端 2s 心跳（`server_ping`） | 3.91 / 5.91 / 7.91 / 9.91 / 11.91s **准点到** |
| 同一轮 run 的 `text/delta` | **28.69s 内逐帧到达**（1.65s → 30.35s，208 帧）|
| 并行的 POST SSE（同一轮 run） | 仍是 32.9s 后一次性到齐 |

落地方式（`web/src/lib/wsStream.ts`）：把 WS 帧**重新编码成 SSE 文本**塞进一个
`Response`，于是 `consumeSSE` / 合帧器 / seq 去重门 / 重连调度**全都不用改**，
只换了字节来源。配套两点：
- `POST /api/sessions` 的响应头既然拿不到及时，就不能靠它拿 `session_id`：
  改成「发起前取会话列表基线 → 发 POST 不 await → 短窗（1200ms）内返回的只可能是
  非流式的 4xx → 否则用列表差分认领新会话 → WS 接流」（`discoverNewSessionId`）。
- 取消 / 断线语义不变：WS 的 `done` 帧即原 SSE 的收流，`server_ping` 要回 `pong`
  （30s 无上行会被判死）。
